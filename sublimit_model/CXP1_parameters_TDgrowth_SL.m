...
function [timescale, Lengthscale, flow_scale, metab_coeff, mu, Ubar, Hh, Kc,Kw, rho, sigma,dcm,dch,dwm,dwh, lambdaM, lambdaH, C_uptake_rate,p] = CXP1_parameters_TDgrowth_SL(proliferation_rate, uptake_rate)
...
if strcmpi(uptake_rate, 'low') == 1
    C_uptake_rate = 9.4e-18; % uptake rate per second   - m^2 per cell per second
elseif strcmpi(uptake_rate, 'mid') == 1
    C_uptake_rate = 9.4e-17; % 
elseif strcmpi(uptake_rate, 'high') == 1  
    C_uptake_rate = 9.4e-16; % 
elseif strcmpi(uptake_rate, 'midx5') ==1
    C_uptake_rate = 5*9.4e-17;
end


p = p_rate*timescale;
rho=N0*C_uptake_rate*timescale/metab_coeff; % where nuc = C_uptake_rate/metab_coeff
sigma=2*rho;
